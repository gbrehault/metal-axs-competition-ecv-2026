import Image from 'next/image';
import Link from 'next/link';
import { toPlainText } from '@/app/lib/utils';

type PostWithCategories = {
  id: string;
  slug: string;
  title: string;
  featuredImage?: {
    node?: {
      altText?: string | null;
      sourceUrl?: string | null;
    } | null;
  } | null;
  categories?: {
    nodes: { name: string; slug: string }[];
  };
  article?: {
    texteIntroduction?: string | null;
    imageDeMiseEnAvant?: {
      node?: {
        sourceUrl?: string | null;
        altText?: string | null;
      } | null;
    } | null;
  } | null;
};

export default function ArticleCard({ post }: { post: PostWithCategories }) {
  const img =
    post.article?.imageDeMiseEnAvant?.node?.sourceUrl || post.featuredImage?.node?.sourceUrl || '';
  const alt =
    post.article?.imageDeMiseEnAvant?.node?.altText || post.featuredImage?.node?.altText || '';
  const title = post.title;
  const texteIntroduction = toPlainText(post.article?.texteIntroduction ?? '');
  const category = post.categories?.nodes[0]?.name?.toUpperCase() ?? 'ARTICLE';

  return (
    <Link
      href={`/bonnes-pratiques/${post.slug}`}
      aria-label={title}
      className="group flex flex-col gap-3 focus-visible:outline-2 focus-visible:outline-primary focus-visible:outline-offset-2"
    >
      <div className="relative w-full aspect-[4/3] overflow-hidden bg-secondary/10 rounded-2xl">
        <Image
          src={img}
          alt={alt}
          fill
          className="object-cover grayscale group-hover:grayscale-0 transition-all duration-500"
          sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw"
        />
        <div className="w-full h-full bg-secondary/20" aria-hidden="true" />
      </div>

      <div className="flex flex-col gap-1">
        <span className="text-xs font-medium text-secondary/50 uppercase tracking-widest">
          {category}
        </span>
        <h5 className="text-xl font-bold font-primary text-secondary leading-snug group-hover:text-primary transition-colors font-primary">
          {title}
        </h5>
        {texteIntroduction && (
          <p className="text-secondary/40 line-clamp-2 leading-relaxed tracking-wide mt-1">
            {texteIntroduction}
          </p>
        )}
      </div>
    </Link>
  );
}

export type { PostWithCategories };
